...
def make_envs(cfg, num_envs, record_video_path, is_eval, logger):
    ...
    """
    # Link save_video to config instead of hardcoding True
    save_vid = getattr(cfg, "save_video", False)
    record_episode_kwargs = dict(save_video=save_vid, save_trajectory=False, record_single=True, info_on_video=False)
    ...
    env_make_fn = partial(
        ...
        id=cfg.env_id, 
        obs_mode=cfg.obs, 
        render_mode="rgb_array", # Hardcode to rgb_array to prevent crashing
        sensor_configs=dict(width=getattr(cfg, 'render_size', 256), height=getattr(cfg, 'render_size', 256))
        )
    if getattr(cfg, 'control_mode', 'default') != 'default':
    ...
    if is_eval: # https://maniskill.readthedocs.io/en/latest/user_guide/reinforcement_learning/setup.html#evaluation
        env_make_fn = partial(env_make_fn, reconfiguration_freq=getattr(cfg, 'eval_reconfiguration_frequency', 1))
    ...
    if cfg.env_type == 'cpu':
        ...
        if cfg['obs'] == 'rgb':
            env = FlattenRGBDObservationWrapper(env, rgb=True, depth=False, state=getattr(cfg, 'include_state', True))
