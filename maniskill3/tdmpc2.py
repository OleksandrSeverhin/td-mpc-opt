...
class TDMPC2:
    ...
    def __init__(self, cfg, teacher=None):
        ...
        # OmegaConf Fix: Store discount as primitives (list or float) in cfg
        if self.cfg.multitask:
            discounts = [self._get_discount(ep_len) for ep_len in cfg.episode_lengths]
            self.discount = torch.tensor(discounts, device='cuda')
            self.cfg.discount = discounts 
        else:
